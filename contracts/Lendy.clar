...
;; (define-constant SWAP-HELPER-CONTRACT 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-swap-helper-v-1-3)
;; (define-constant POOL-CONTRACT 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-sbtc-stx-v-1-1)
...
(define-public (deposit  (deposit-nft <nft-tokens>) (amount uint))
  (let (
    ...
    (var-set total-stx-deposit (+ (var-get total-stx-deposit) amount))
     ...
     (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.Lendo_Nft mint tx-sender))
...
(let (
    ...
    (asserts! (>= existing-amount amount) ERR_INVALID_WITHDRAW_AMOUNT)
     ...
     (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.Lendx_Nft mint user))
  ...
  (let (
    ...
    (var-set total-sbtc-collateral (+ (var-get total-sbtc-collateral) sbtc-amount))
     ...
     (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer 
      ...
      (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.LenBo_Nft mint user))
...
(define-public (repay (repay-nft <nft-tokens>))
   (let (
     ...
     (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer deposited-sbtc (as-contract tx-sender) tx-sender none))
      ...
      (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.Lendy_Nft mint tx-sender))
    ...
    (let (
      ...
      (asserts! (<= (* collateral-value-in-stx u100)
        ...
        swap-helper-a pool-reward u0 none xyk-tokens xyk-pool
         ...
         (let ((lenBo-token-id (unwrap-panic (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.LenBo_Nft get-last-token-id)))
               (lendo-token-id (unwrap-panic (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.Lendo_Nft get-last-token-id))))

       ;; burn the borrow nft to the user
        (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.LenBo_Nft burn lenBo-token-id))
        ;; burn the borrow nft to the user
      (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.Lendo_Nft burn lendo-token-id))
...
;;get the total stx deposited
(define-read-only (get-total-stx-deposit)
  (ok (var-get total-stx-deposit))
)

;;get the total stx borrowed
(define-read-only (get-total-stx-borrowed)
  (ok (var-get total-stx-borrowed))
)
;;get the total sbtc collateral
(define-read-only (get-total-sbtc-collateral)
  (ok (var-get total-sbtc-collateral))
)
;;get the protocol fee
(define-read-only (get-protocol-fee)
  (ok (var-get protocol-fee))
)
;;get the cumulative interest yield
(define-read-only (get-cumulative-interest-yield)
  (ok (var-get cumulative-interest-yield))
)
;;get the last interest accrual
(define-read-only (get-last-interest-accrual)
  (ok (var-get last-interest-accrual))
)
;;get user deposits 
...
(define-read-only (get-user-borrow (user principal))
  (ok (map-get? borrows {user: user}))
)
;;get user collateral
(define-read-only (get-user-collateral (user principal))
  (ok (map-get? collateral {user: user}))
)
;;get LVT percentage
(define-read-only (get-LVT-percentage)
  (ok LVT_PERCENTAGE)
)
;;get interest rate percentage
(define-read-only (get-interest-rate-percentage)
  (ok INTEREST_RATE_PERCENTAGE)
)
;;get liquidation threshold percentage
(define-read-only (get-liquidation-threshold-percentage)
  (ok LIQUIDATION_THRESHOLD_PERCENTAGE)
)

;;get protocol fee percentage
(define-read-only (get-protocol-fee-percentage)
  (ok PROTOCOL_FEE_PERCENTAGE)
)